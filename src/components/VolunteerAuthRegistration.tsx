// components/VolunteerAuthRegistration.tsx - Fixed
import React, { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { Mail, Lock, User, CheckCircle, AlertCircle, Users } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { validateEmail, validatePassword, validateConfirmPassword, validateName } from '../utils/validation';

interface VolunteerAuthRegistrationProps {
  onSuccess: () => void;
}

export const VolunteerAuthRegistration: React.FC<VolunteerAuthRegistrationProps> = ({ onSuccess }) => {
  const navigate = useNavigate();
  const { signUpVolunteer, setRegistrationState } = useAuth(); // Fixed: use destructured name
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Starting volunteer auth registration via Edge Function...');
      
      // Send ONLY the required fields - Edge Function will handle the rest
      const result = await signUpVolunteer(formData.email, formData.password, {
        first_name: formData.firstName.trim(),
        last_name: formData.lastName.trim()
        // REMOVE all other fields - Edge Function will set role = 'volunteer'
      });
  
      console.log('Volunteer auth registration completed:', result);
  
      if (result.success) {
        console.log('Volunteer auth registration successful, showing success message...');
        
        // Set registration state explicitly to ensure it's tracked
        setRegistrationState({
          hasAuth: true,
          role: 'volunteer', // Default role
          profileComplete: false, // Profile is not complete
        ...
        setTimeout(() => {
          console.log('Navigating to volunteer registration...');
          onSuccess();
        }, 1500);
      } else {
        console.error('Volunteer auth registration failed:', result.error);
      ...
      console.error('Unexpected volunteer registration error:', error);
      setErrors([{ 
        field: 'general', 
